"""
You can debug the color recognition 
by changing the display_thread function 
to show only the color masks instead of 
combining everything to one frame or find 
the contours from specific mask phases such 
as mask, binary_mask, object_mask, clean_mask., etc.

Very handy tool when used with the color_finder.py
to find the right HSV values for the colors you want to detect.
"""
