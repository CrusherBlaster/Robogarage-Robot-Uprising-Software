...
def draw_worker(contour_queue, aruco_queue, draw_queue, display_queue, stop_event_mp):
    while True:
        ...
        try:
            display_frame = draw_queue.get(timeout=0.1)
            ball_info = contour_queue.get(timeout=0.1)
            aruco_detections = aruco_queue.get(timeout=0.1)
        except queue.Empty:
            if stop_event_mp.is_set() and draw_queue.empty() and contour_queue.empty() and aruco_queue.empty():
                break
            continue

        # Stop if sentinel received from any queue
        if display_frame is None or ball_info is None or aruco_detections is None:
            break

        frame_with_balls = draw_ball_center(ball_info, display_frame)
        finished_drawing = draw_arucos(aruco_detections, frame_with_balls)
    ...
    for role, colors in object_information["arena_objects"]["balls"].items():
        for color, shades in colors.items():
            for shade, info in shades.items():
                ...
                morphed_mask = cv2.morphologyEx(
                    morphed_mask, cv2.MORPH_OPEN, np.ones((3, 3), np.uint8))
                # Converting to black & white for data reduction & debugging
                _, morphed_binary_mask = cv2.threshold(
                    morphed_mask, 127, 255, cv2.THRESH_BINARY)
                info["masks"].update({
                    "mask": mask,
    ...
    for role, colors in object_information["arena_objects"]["balls"].items():
        for color, shades in colors.items():
            for shade, info in shades.items():
                ...
                binary_contours, hierarchy = cv2.findContours(
                    morphed_binary_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                ...
                for i, binary_contour in enumerate(binary_contours):
                    ...
                    if area > 300:
                        ...
                        cv2.drawContours(
                            object_mask, [binary_contour], -1, 255, cv2.FILLED)
                ...
                clean_contours, rejected = cv2.findContours(
...
if __name__ == "__main__":
    ...
    p_mask = mp.Process(target=mask_worker, args=(
        hsv_queue, mask_queue, object_information, stop_event_mp))
    p_contour = mp.Process(target=contour_worker, args=(
        mask_queue, contour_queue, stop_event_mp))
    p_aruco = mp.Process(target=aruco_worker, args=(
        gray_queue, aruco_queue, stop_event_mp))
    p_draw = mp.Process(target=draw_worker, args=(
        contour_queue, aruco_queue, draw_queue, display_queue, stop_event_mp))
    ...
    display_thread = threading.Thread(
        target=display_frames, args=(display_queue, ))
