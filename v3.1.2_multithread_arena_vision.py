...
TEST_IMAGE_PATH = '/mnt/data/2fcd5f99-f84b-430a-ac52-b75914bbfa95.png'  # developer-provided file
...
PEAKS_OPEN_KERNEL = np.ones((3, 3), np.uint8)

# Contour filters and expected sizes (in pixels) - tune according to camera distance
MIN_CONTOUR_AREA = 250
MAX_CONTOUR_AREA = 100000

DIST_PEAK_FACTOR = 0.3

# circularity threshold for "clear single ball" decision
...
ball_count = 0
debug = np.zeros((FRAME_H, FRAME_W, 3), dtype=np.uint8)
...
def process_and_display():
    ...
    where it matters.
    """
    global latest_frame, latest_corners, latest_ids, balls_tracked, next_local_ball_id, ball_count, stop_requested, debug
    ...
    while not stop_requested:
        ...
        for color_name, (lower, upper) in HSV_RANGES.items():
            ...
            # Morphology: closing fills hole-punctures; opening removes small specks
            closed = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, MORPH_CLOSE_KERNEL)
            opened = cv2.morphologyEx(closed, cv2.MORPH_OPEN, MORPH_OPEN_KERNEL)
            ...
            for contour in contours:
                ...
                if circularity >= CIRCULARITY_THRESHOLD and 20 < radius < 120:
                    print("Sufficiently circular: ", color_name)
                    all_detections.append((circle_center_x, circle_center_y, radius, color_name))
                    continue
                
                # print("Watershedding: ", color_name)
                # Otherwise, try splitting the region into multiple balls using watershed
                # Compute the axis-aligned bounding box of the contour.
                # boundingRect() works in image coordinates, where:
                #   - (0,0) is the upper-left corner of the full image,
                #   - x increases to the right,
                #   - y increases downwards.
                #
                # The function scans all contour points and finds:
                #   rect_up_l_x = minimum x-value among contour points
                #   rect_up_l_y = minimum y-value among contour points
                #   rect_width = (maximum x-value - minimum x-value)
                #   rect_height = (maximum y-value - minimum y-value)
                #   -> 2 points one closest to the img origin and one furthest away from it
                # The returned rectangle is thus the smallest axis-aligned rectangle
                ...
                roi_mask = opened[row_y0:row_y1, column_x0:column_x1]
                ...
                expected_radius = 10
                fit_mask = (dist >= expected_radius).astype(np.uint8) * 255
                debug = fit_mask
                ...
                threshold = DIST_PEAK_FACTOR * dist.max()
                _, peaks = cv2.threshold(dist_norm, DIST_PEAK_FACTOR, 1.0, cv2.THRESH_BINARY)
                ...
                peaks = cv2.morphologyEx(peaks, cv2.MORPH_OPEN, PEAKS_OPEN_KERNEL)
