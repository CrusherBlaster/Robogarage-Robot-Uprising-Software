...
def detect_colors(hsv):
    ...
    orange_mask = cv2.inRange( hsv, (5,90,110), (25,255,255) )
...
def display_thread():
    while True:
        ...
        for color, mask in latest_colors.items():
            ...
            contours, rejected = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            object_mask = np.zeros_like(mask)
            clean_mask = None
            for contour in contours:
                if cv2.contourArea(contour) > 300: # ignore tiny blobs
